...
class BYOL(nn.Module):
    def __init__(self, net, image_size, hidden_layer = -2, projection_size = 256, projection_hidden_size = 4096, augment_fn = None, augment_fn2 = None, moving_average_decay = 0.99, use_momentum = True):
        ...
        self.reset_binary_gates(0, 0, True)
        self.unused_ops_off()
        self.forward(torch.randn(2, 3, image_size, image_size, device=device))
        self.unused_ops_back()
    ...
    def save_arch_weights(self, save_path, note):
        self.net.save_arch_weights(save_path, note)
    
    def get_cells_arch(self, epoch, add_restrict):
        self.net.get_cells_arch(epoch, add_restrict)
    
    def reset_binary_gates(self, skip_drop_prob, pool_drop_prob, warmup):
        self.net.reset_binary_gates(skip_drop_prob, pool_drop_prob, warmup)

    def unused_ops_off(self):
        self.net.unused_ops_off()

    def unused_ops_back(self):
        self.net.unused_ops_back()
    ...
    def progressive_prune(self, args, epoch):
        self.net.progressive_prune(args, epoch)
